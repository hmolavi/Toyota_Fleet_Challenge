...
class Robot(Node):
    def __init__(self):
        ...
        path_image = '/oakd/rgb/preview/image_raw'
        path_CameraInfo = '/oakd/rgb/preview/camera_info'

        global is_SIM
        if (is_SIM):
            print("using sim path_CameraInfo")
            path_CameraInfo = '/camera/camera_info'
            print("using sim path_image")
            path_image = '/camera/image_raw'

        self.image_subscription = self.create_subscription(Image, path_image, self.image_listener_callback,qos_profile_sensor_data)

        self.image_subscription  # prevent unused variable warning
        
        self.camera_info_future = rclpy.Future()

        self.camera_info_subscription = self.create_subscription(CameraInfo, path_CameraInfo, self.camera_info_listener_callback,qos_profile_sensor_data)
